...
const Order = () => {
  ...
  useEffect(() => {
    const fetchPreviousOrders = async () => {
      const fetchPreviousOrders = await get(
        `/customers/previous-orders/${id}`,
      ).then((response) => {
  ...
  return (
    <>
      <div className="relative">
        <button
          className="bg-croonus-4 p-[0.4rem] sm:mr-[4rem] max-sm:mr-[0.6rem] ml-[1re] rounded-[50%] mt-[0.4rem] hover:translate-y-0.5 transition-all ease cursor-pointer text-white min-w-[2.2rem] absolute md:top-4 md:left-4"
      ...
      <div className="mb-[4rem]">
        <div className="bg-[#f8f8f8] rounded-lg p-[1.4rem] md:w-[90%] mb-[3rem]">
          <h1 className="text-3xl pb-0 mb-[1rem] ml-[3rem]">
            Detalji porudžbenice
          </h1>
          <h3 className="text-base text-[#919191] font-normal max-sm:ml-[4rem]">
            Porudžbenica{" "}
            <span className="font-medium">{previousOrders?.order?.slug}</span>
          </h3>
        </div>
        <div className="flex">
          <p className="font-light bg-croonus-gray px-[1.4rem] py-[0.8rem] rounded-lg">
            Kreirana:{" "}
            <span className="font-medium">
              {previousOrders?.order?.created_at}
            </span>
          </p>
          <p className="font-light bg-croonus-gray px-[1.4rem] py-[0.8rem] rounded-lg mx-[0.6rem]">
            Način dostave:{" "}
            <span className="font-medium">
              {previousOrders?.order?.delivery_method}
            </span>
          </p>
          <p className="font-light bg-croonus-gray px-[1.4rem] py-[0.8rem] rounded-lg">
            Način plaćanja:{" "}
            <span className="font-medium">
              {previousOrders?.order?.payment_method}
            </span>
          </p>
        </div>
          ...
          <table className="table-fixed max-md:w-full w-[90%] mr-auto my-1rem table max-sm:hidden">
            <tbody>
              <tr className="bg-croonus-2 divide-x divide-white text-white">
                <td
                  className=" rounded-tl-lg pl-[1.4rem] py-[0.7rem]"
                  colSpan="2"
                >
                  Proizvod
                </td>
                <td className=" pl-[1.4rem] py-[0.7rem]">SKU</td>
                <td className=" pl-[1.4rem] py-[0.7rem]">Cena</td>
                <td className=" pl-[1.4rem] py-[0.7rem]">Količina</td>
                        ...
                        <div className="flex my-[0.2rem] items-center">
                          ...
                          <div>
                            <h5 className="text-base font-medium">
                              {order?.basic_data?.name}
                            </h5>
                            <p className="text-sm text-[#939393]">
                              {order?.basic_data?.brand_name}
                            </p>
                          </div>
                        </div>
                      </td>
              ...
              return (
                <>
                  <div className="border rounded-lg border-[#f0f0f0] px-[1rem] pb-[1rem] mb-[2rem]">
                    ...
                    <p className="text-sm text-[#939393] border-b border-croonus-3">
                      {order?.brand_name}
                    </p>
                    <p className="font-light mt-[1rem]">
                      SKU: <span className="font-medium">{order?.sku}</span>
                    </p>
                    <p className="font-light mt-[0.2rem]">
                      {" "}
                      Cena:{" "}
                      <span className="font-medium">
                        {order?.cost?.per_item?.with_vat}
                      </span>
                    </p>
                    <p className="font-light mt-[0.2rem]">
                      {" "}
