...
import { useIsInWishlist, useProductThumb } from "@/hooks/akt.hooks";
...
const Products = ({
  ...
  const { data: item } = useProductThumb({
    slug: slug,
    categoryId: categoryId,
    id: slug,
  });

  const { data } = useIsInWishlist({ id: id });

  const isProductInWishlist = data?.exist;
        ...
        <Link className={`w-full`} href={`/${item?.link?.link_path}`}>
          <div className="relative w-full">
            ...
            {item?.price?.discount?.active && (
              ...
              >
                <p className={`text-black`}>{renderDiscountPercentage(item)}</p>
              </div>
            )}
          </div>
        </Link>
